# Extracted from <https://github.com/mbello/dict-deep>
# Is not included as we do not want to depend on PyPI for a very simple package.
# Improved in this project for typing hints.

from typing import Dict, TypeVar, Optional, List, Callable, Union

KT = TypeVar('KT')
VT = TypeVar('VT')
OT = TypeVar('OT')
AccessorT = Callable[[Dict[KT, VT], KT], Union[VT, OT]]

def deep_get(o: Dict[KT, VT], k: KT, accessor: AccessorT = dict.get,
             getter: callable = None, getter_last_step: callable = None, sep: str = '.',
             empty_list_as_none: bool = False, list_of_len_one_as_value: bool = False) -> Union[VT, OT]:
    keys = __keys(k, sep)
