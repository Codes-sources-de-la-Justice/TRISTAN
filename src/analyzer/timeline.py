...
def reconcile_person_field(key, old_person, new_person):
...
def reconcile_persons(old, new):
    ...
    for key_to_merge in to_merge:
        ...
        for field_key in old_p.keys() + new_p.keys():
            final[key_to_merge][field_key] = reconcile_person_field(field_key, old_p, new_p)
...
class BagOfFacts:
    ...
    def ingest(self, piece, persons) -> bool:
        ...
        fait = data.get('Procedure', {}).get('Faits', {}).get('Fait', {})
        if 'Natinf' in fait:
            p_metadata = nature_of(piece)
            self.create_or_update({
                'metadata': p_metadata,
                'start_utc': fait['Periode_Affaire_Debut']['@utc'],
                'end_utc': fait.get('Periode_Affaire_Fin', {'@utc': None})['@utc'],
                'tentative': True if fait['Fait_Tentative'] == 'OUI' else False,
                'natinf': fait['Natinf'],
                'qualification': fait.get('Fait_Qualification', 'INCONNUE'), # est ce nécessaire, compte tenu du NATINF?
                'mode_operatoire': fait.get('Maniere_Operer'),
                'source': src,
                'position': (fait.get('Fait_GpsX'), fait.get('Fait_GpsY'))
            })
...
class GraphContext:
    ...
    def ingest(self, piece) -> bool:
        ...
        self.extend_persons(PersonRole.VICTIM, find_victims(data))
        self.extend_persons(PersonRole.AUTHOR, find_authors(data))
        self.extend_persons(PersonRole.WITNESS, find_witness(data))
    ...
    def create_graph(self):
        ...
        # - lier.
        return {'nodes': metadata, 'adjacency_list': graph}


    def extend_persons(self, nature, new):
        self.persons[nature] = reconcile_persons(self.persons.get(nature, []), new)
